...
impl Node {
    pub fn generate_argument(&self, env: &Scope) -> CrushResult<ArgumentDefinition> {
        Ok(ArgumentDefinition::unnamed(match self {
            ...
            Node::Cast(value, target_type) => {
                ValueDefinition::JobDefinition(Job::new(vec![CommandInvocation::new(
                    ValueDefinition::Value(Value::Command(
                        env.global_static_cmd(vec!["global", "types", "as"])?,
                    )),
                    vec![
                        value.generate_argument(env)?,
                        target_type.generate_argument(env)?,
                    ],
                )]))
            }
    ...
    fn function_invocation(
        function: Box<dyn CrushCommand + Send + Sync>,
