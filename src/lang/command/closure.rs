...
use crate::lang::command::{BoundCommand, CrushCommand, Parameter};
...
impl<'a> ClosureDeserializer<'a> {
    ...
    pub fn closure(&mut self, id: usize) -> CrushResult<Box<dyn CrushCommand + Send + Sync>> {
    ...
    fn parameter(&mut self, parameter: &model::Parameter) -> CrushResult<Parameter> {
        match &parameter.paremeter {
            ...
            Some(model::parameter::Paremeter::Normal(param)) => Ok(Parameter::Parameter(
                param.name.clone(),
            ...
            Some(model::parameter::Paremeter::Unnamed(param)) => {
