...
mod tests {
    ...
    fn allowed_values() {
        ...
        assert!(AllowedValuesStringSignature::parse(
            vec![
                Argument::named("str_val", Value::string("zz")),
            ],
            &printer,
        ).is_err());
        ...
        let a = AllowedValuesCharSignature::parse(
            vec![
                Argument::named("char_val", Value::string("a")),
            ],
            &printer,
        ).unwrap();
        ...
        assert!(AllowedValuesCharSignature::parse(
            vec![
                Argument::named("char_val", Value::string("z")),
            ],
            &printer,
        ).is_err());
        ...
        let a = AllowedValuesIntSignature::parse(
            vec![
                Argument::named("int_val", Value::Integer(1)),
            ],
            &printer,
        ).unwrap();
    ...
    fn list_signature() {
        ...
        assert_eq!(ListSignature::parse(
            ...
            &printer,
        ).unwrap().list_val, vec!["a".to_string(), "b".to_string(), "c".to_string()]);

        assert_eq!(ListSignature::parse(
                vec![],
            &printer,
        ).unwrap().list_val, Vec::<String>::new());

        assert_eq!(ListSignature::parse(
            vec![
                Argument::named("list_val", Value::string("a")),
                Argument::named("list_val", Value::List(List::new(
                    ValueType::String,
            ...
            &printer,
        ).unwrap().list_val, vec!["a".to_string(), "b".to_string(), "c".to_string(), "d".to_string()]);
