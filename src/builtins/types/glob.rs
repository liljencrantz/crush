...
#[signature(
    ...
    short = "True if the needle matches this glob. Not meant to be called directly -- use the `like` command or the `=~` operator.",
)]
struct Is {
...
#[signature(
    ...
    can_block = false,
    output = Known(ValueType::Bool),
    short = "False if the needle matches this glob. Not meant to be called directly -- use the `like` command or the `!~` operator.",
)]
struct IsNot {
    #[description("the text to match this glob against.")]
    needle: Text,
