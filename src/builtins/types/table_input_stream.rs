...
#[signature(
    ...
    long = "",
    long = "Always `fg` every writer job before calling `pipe:close` (readers don't need this --",
    long = "only writers). `pipe:write` reads the pipe's internal state on its own worker thread",
    long = "whenever it happens to get scheduled, with no synchronization against `pipe:close`;",
    long = "calling `pipe:close` before a writer job has actually had a chance to run can silently",
    long = "drop that writer's entire contribution.",
    example = "# Create a pipe",
    example = "$pipe := $($(table_input_stream value=$integer):pipe)",
    example = "# Create a job that writes 100_000 integers to the pipe and put this job in the background",
    example = "$write_job_handle := $(seq 0 100_000 | pipe:write &)",
    ...
    example = "# Put the writer job in the foreground so it fully finishes before the pipe is closed",
    example = "fg $write_job_handle",
