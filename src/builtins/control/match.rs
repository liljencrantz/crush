...
#[signature(
    ...
    long = "The body is a block containing a sequence of arms, each one of:",
    long = "* `case <value> {<body>}` -- matches if the subject equals `<value>`.",
    long = "* `any <stream> {<body>}` -- matches if the subject equals any single value",
    ...
    long = "skipped. If nothing matches, match errors, unless a `default` arm was given.",
    example = "match $x {",
    example = "    case 10 {echo bar}",
    example = "    any $(seq 5 10) {echo foo}",
    example = "    is $string {echo baz}",
    example = "    default {echo qux}",
    example = "}",
)]
pub struct Match {
